...
const TripleForm = ({
  ...
  const onAddToLeftOfSubjectType = (element, innerIndex) => {
    invalidAddition(element, () => {
      addSubjectLeft(element, index ?? 0, innerIndex);
    });
  };

  const onAddToLeftOfObjectType = (element, innerIndex) => {
    invalidAddition(element, () => {
      addObjectLeft(element, index ?? 0, innerIndex);
    });
  };

  const onAddToRightOfSubjectType = (element, innerIndex) => {
    invalidAddition(element, () => {
      addSubjectRight(element, index ?? 0, innerIndex);
    });
  };

  const onAddToRightOfObjectType = (element, innerIndex) => {
    invalidAddition(element, () => {
      addObjectRight(element, index ?? 0, innerIndex);
    });
  };
        ...
        <ChipsContainer>
          ...
          {data?.contextList?.map((item, i) => {
            return (
              <Chip
                content={[
                  { labelKey: item.context, labelValue: item.contextValue },
                ]}
                onDelete={() => {
                  deleteAddedContext(i);
                }}
              />
            );
          })}
            ...
            <Grid item xs={9}>
              {data?.code?.trim() !== "" ? (
                <ChipsContainer>
                  ...
                  {data.flagged === true ? (
                    <Tooltip
                      message={`Flagged Comment : ${data.comment}`}
                      position="top"
                    >
                      <IconButton
